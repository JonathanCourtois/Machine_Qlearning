...
class underwater() :
    ...
    def actionMemoriesCheck(self,nb):
        test = np.array(self.actionMemories)
        bugAlert = False
        if (np.sum((test==nb)*1)>9):
            bugAlert = True
        if (len(self.actionMemories)>9):
            self.actionMemories.pop(0)
            
            self.actionMemories.append(nb)
        return bugAlert   
    ...
    def updateScore(self):
        ...
        dist_factor = ((dist)/18)
        turn_factor = (self.count_turn/20)
        life_factor = (3-self.player[0].life)*10
        
        self.score = 1 - dist_factor - turn_factor - life_factor
...
if (train_deep_Q):
    ...
    agent = Agent(gamma=0.99, epsilon = 1, batch_size=int(n_games/10), n_actions = 5,
                  eps_end=0.01, input_dims=[105], lr=0.001)
