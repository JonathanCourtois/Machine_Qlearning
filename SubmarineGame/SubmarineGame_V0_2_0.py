...
class Signal(pygame.sprite.Sprite):
    ...
    def move(self, game):
        ...
        while(not(move)):
            # saving old position
            oldPosX = self.position[0]
            oldPosY = self.position[1]
            # choose alea index 0 or 1 for x or y
            index =  np.random.randint(2) # int alea  0 or 1
            # saving the x or y value
            tmp = self.position[index]
            #add +1, 0 or -1
            self.position[index] += np.random.randint(3)-1
            # if we didn't move in the x or y chossen by the index
            if (self.position[index] == tmp):
                # the other axis move
                self.position[1-index] += np.random.randint(2)*2-1
            # check border limit
            self.border()
            move = True
            for m in game.mine:
                if (self.position[0] == m.position[0] and self.position[1] == m.position[1]):
                    move = False
                    # resoring old position
                    self.position[0] = oldPosX
                    self.position[1] = oldPosY
        ...
        while not self.endFlag:  
            ...
            for event in pygame.event.get():
                if event.type == pygame.QUIT:
                    ...
                    if(moove):
                        observation_, self.score, self.endFlag, info = game.step(action_id)
                        agent.store_transition(observation_, action_id, self.score, 
